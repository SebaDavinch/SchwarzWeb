...
const howItWorks = [
  {
    ...
    desc: "Проводи время с участниками семьи на сервере. Учствуй в совместных активностях, покажи, что ты свой.",
  },
  {
    step: "02",
...
function ApplicationForm() {
  ...
  const validate = (): boolean => {
    ...
    if (!form.nickname.trim()) errs.nickname = "Обязательное пол";
    ...
    if (!form.readyToRename) errs.readyToRename = "Обязательное поле";
    setErrors(errs);
  ...
  return (
    ...
    >
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        {/* Готовы сменить фамилию */}
        <div>
          <label
            className="font-['Oswald'] text-white/25 uppercase tracking-wider flex items-center gap-2 mb-3"
            style={{ fontSize: "0.6rem" }}
          >
            <User size={10} className="text-white/15" />
            Готовы ли вы сменить фамилию на Schwarz?
            <span className="text-[#ff3366]/40">*</span>
          </label>
          <div className="flex gap-3">
            {([
              ...
              <button
                key={opt.value}
                type="button"
                onClick={() => updateField("readyToRename", opt.value)}
                className={`flex items-center gap-2.5 px-5 py-3 border transition-all duration-300 ${
                  form.readyToRename === opt.value
                    ? "border-[#9b2335]/30 bg-[#9b2335]/[0.06]"
              ...
              >
                ...
                <span
                  className={`font-['Oswald'] tracking-wide transition-colors duration-300 ${
                    form.readyToRename === opt.value
                      ? "text-[#9b2335]/80"
                      : "text-white/35"
                  }`}
