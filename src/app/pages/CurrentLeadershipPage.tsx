...
const activities = [
  ...
  },
  {
    ...
    desc: "Набираем людей, обучаем, планируем. Когда пойдём на следующую лидерку — будем готовы.",
  },
];

export function CurrentLeadershipPage() {
  const { getPageOverride } = useAdminData();
  const heroTitle = getPageOverride("current-leadership", "hero_title") ?? "Сейчас — отдыхаем";
  const heroTitlePlain = heroTitle.replace(/<[^>]*>/g, " ").replace(/\s+/g, " ").trim() || "Сейчас — отдыхаем";
  ...
  const statusLabel = getPageOverride("current-leadership", "status_label") ?? "На паузе";
  const activitiesTitle = getPageOverride("current-leadership", "activities_title") ?? "Между лидерками";
  const activitiesDesc = getPageOverride("current-leadership", "activities_desc") ?? "Чем заняты";
