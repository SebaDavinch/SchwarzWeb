...
import { addAuditLog, useMoments, type Moment, type MomentCategory, MOMENT_CATEGORIES } from "../../hooks/useAdminData";
...
export function MomentsTab() {
  const { moments, setMoments } = useMoments();
  ...
  const handleDelete = (id: string) => {
    ...
    setMoments(moments.filter((x) => x.id !== id));
    if (m) addAuditLog("Момент удалён", "media", m.title);
  };

  const toggleFeatured = (id: string) => {
    setMoments(moments.map((m) => (m.id === id ? { ...m, featured: !m.featured } : m)));
  };

  const toggleVisible = (id: string) => {
    setMoments(moments.map((m) => (m.id === id ? { ...m, visible: !m.visible } : m)));
  };
