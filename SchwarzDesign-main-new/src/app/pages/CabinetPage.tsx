...
import {
  ...
  BookOpen, Camera, Shield, AlertTriangle, Target,
  Image as ImageIcon, CalendarDays, Wallet,
} from "lucide-react";
...
import { CabinetGoals } from "../components/cabinet/CabinetGoals";
import { CabinetReports } from "../components/cabinet/CabinetReports";
import { CabinetCalendar } from "../components/cabinet/CabinetCalendar";
import { CabinetTreasury } from "../components/cabinet/CabinetTreasury";
import { Navbar } from "../components/Navbar";
import { SPECIALIZATIONS } from "../components/cabinet/CabinetProfile";

type Tab =
  | "feed" | "profile" | "upgrades" | "infrastructure"
  | "vehicles" | "contracts" | "achievements" | "glossary"
  | "goals" | "reports" | "calendar" | "treasury";
...
const TABS: { id: Tab; label: string; Icon: typeof Bell }[] = [
  ...
  { id: "treasury",       label: "Казна",          Icon: Wallet },
];
...
export function CabinetPage() {
  ...
  const handleCoverChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0]; if (!file) return;
    const reader = new FileReader();
    reader.onload = ev => updateAccount(currentAccount.id, { coverDataUrl: ev.target?.result as string });
    reader.readAsDataURL(file);
  };
        ...
        <div className="flex flex-col lg:flex-row gap-0">
          ...
          <div className="lg:w-[230px] shrink-0 -mt-[72px] z-10 self-start mb-4 lg:mb-0">
            <div className="bg-[#12121b] border border-white/[0.07] p-5 flex flex-col items-center text-center gap-4">
              ...
              {/* Specialization tags */}
              {(() => {
                const specs: string[] = (currentAccount as any).specializations ?? [];
                if (specs.length === 0) return null;
                const found = SPECIALIZATIONS.filter(s => specs.includes(s.id));
                return (
                  <div className="w-full">
                    <p className="text-white/15 uppercase tracking-widest mb-2 text-left"
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {found.map(sp => (
                        <div key={sp.id}
                          ...
                          style={{ background: `${sp.color}10`, border: `1px solid ${sp.color}30` }}>
                          <span style={{ fontSize: "0.75rem" }}>{sp.icon}</span>
                          <span className="uppercase tracking-wider"
                            style={{ fontFamily: "'Oswald',sans-serif", fontSize: "0.5rem", color: sp.color }}>
                            {sp.label}
                          </span>
                        </div>
                      ))}
            ...
            <div className="py-6">
              <AnimatePresence mode="wait">
                ...
                >
                  ...
                  {activeTab === "goals" && <CabinetGoals />}
                  {activeTab === "reports" && currentAccount && <CabinetReports account={currentAccount} />}
                  {activeTab === "calendar" && <CabinetCalendar />}
                  {activeTab === "treasury" && <CabinetTreasury />}
                </motion.div>
              </AnimatePresence>
            </div>
          </div>
